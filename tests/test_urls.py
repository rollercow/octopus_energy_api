...
def test_consumption_url():
    ...
    # without page size
    assert (
        "https://api.octopus.energy/v1/electricity-meter-points/mpan/meters/serial/consumption?page_size=25000&period_from=start&period_to=end&order_by=period"
        == urls.consumption_url("mpan", "serial", "start", "end")
    )

    # with page size
    assert (
        "https://api.octopus.energy/v1/electricity-meter-points/mpan/meters/serial/consumption?page_size=10000&period_from=start&period_to=end&order_by=period"
        == urls.consumption_url("mpan", "serial", "start", "end", page_size=10000)
    )


def test_meter_point_url():

    assert "https://api.octopus.energy/v1/electricity-meter-points/mpan/" == urls.meter_point_url(
        "mpan"
    )
